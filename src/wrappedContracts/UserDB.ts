...
export class UserDB extends GeneralFunctions {
    ...
    async setRoles(_user: string, _roles: number, txParams?: SpecialTx) {
        ...
        const txData = await this.web3Contract.methods.setRoles(_user, _roles).encodeABI();
        ...
        if (txParams) {
            ...
            if (!txParams.gas) {
                try {
                    gas = await this.web3Contract.methods.setRoles(_user, _roles).estimateGas({
                        from: txParams ? txParams.from : (await this.web3.eth.getAccounts())[0]
                    });
                } catch (ex) {
                    if (!(await getClientVersion(this.web3)).includes('Parity')) {
    ...
    async setUserActive(_user: string, _active: boolean, txParams?: SpecialTx) {
        ...
        const txData = await this.web3Contract.methods.setUserActive(_user, _active).encodeABI();
